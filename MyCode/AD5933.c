...
{   
  ...
  //START(); 
	...
	nTemp=IIC_Read_Byte(0);//ReadByte();//读一个字节数据		
	//SendNACK();//发送NO_ACK
...
void AD5933_Init(float Freq, float Freq_Step, u16 Step_Num)
{
	...
	Fre_To_Hex(Freq,SValue); //起始频率
	Fre_To_Hex(Freq_Step,IValue);      //增量频率
	NValue[0]=Step_Num>>8;
	NValue[1]=Step_Num;               //增量数
		...
		for(i = 0;i <3;i++)
    {
      ...
      AddrTemp++;
		...
		AddrTemp=0X81;
		Write_Byte(AddrTemp,0X80);//初始化控制寄存器，1011 0001 0000 0000待机模式，2V，一倍放大，内部时钟 
		
		AddrTemp=0X80;
		Write_Byte(AddrTemp,0X11);
		AddrTemp=0X81;
		Write_Byte(AddrTemp,0X80);//以起始频率初始化，建立激励
		
		delay_ms(10);
		AddrTemp=0X80;
		Write_Byte(AddrTemp,0X21);
		AddrTemp=0X81;
		Write_Byte(AddrTemp,0X80);//启动频率扫描
